...
template <Scalar T>
Octagon<T>::Octagon(const Point<T>& center, T radius) : center_(center), radius_(radius) {
...
template <Scalar T>
Octagon<T>::Octagon(const Octagon& other) : Figure<T>(other), center_(other.center_), radius_(other.radius_) {
    copy_vertices(other);
}
...
template <Scalar T>
double Octagon<T>::surface() const {
...
template <Scalar T>
double Octagon<T>::perimeter() const {
    const double r = static_cast<double>(radius_);
...
template <Scalar T>
bool Octagon<T>::operator==(const Octagon& other) const noexcept {
...
void Octagon<T>::read(std::istream& is) {
    ...
    is >> x >> y >> radius;
    if (!is) {
        throw std::runtime_error("Failed to read octagon data");
    }
...
void Octagon<T>::print(std::ostream& os) const {
    os << "Octagon: center=" << center_ << ", radius=" << radius_ << ", vertices=";
...
template <Scalar T>
void Octagon<T>::copy_vertices(const Octagon& other) {
    for (std::size_t i = 0; i < kVertexCount; ++i) {
